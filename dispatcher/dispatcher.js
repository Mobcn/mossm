...
/**
 * @type {{ [path: string]: () => Promise<{ default: VController }> }}
 */
const router = {
 ...
 */
async function runStatic(path, context) {
    await DB.connect();
 ...
 * @param {Context} context 请求上下文
 */
async function runDynamical(module, model, path, context) {
    await DB.connect();
 ...
 * @param {Context} context 请求上下文
 */
export default async function dispatcher(context) {
    const { pathname } = new URL(context.req.url, 'http://' + context.req.header('host'));
    let run;
    if (Object.keys(router).includes(pathname)) {
        run = runStatic(pathname, context);
    } else {
        const paths = pathname.split('/');
        if (paths.length < 4) {
            return context.text('Not Found', 404, { 'Content-Type': 'text/plain; charset=utf-8' });
        }
        const module = paths[1];
        const model = paths[2].replace(/./, (i) => i.toUpperCase());
        const path = '/' + paths.slice(3).join('/');
        run = runDynamical(module, model, path, context);
    }
    return run.catch((error) => context.text(error.message, 500, { 'Content-Type': 'text/plain; charset=utf-8' }));
}
